...
public class BinanceBackfillService(
    ...
    ILogger<BinanceBackfillService> logger,
    ConcurrentBag<string> symbols,
    PriceHistoryCache priceHistoryCache) : BackgroundService
{
    ...
    private async Task BackfillDataAsync(CancellationToken cancellationToken)
    {
        ...
        var binanceExchange = await dbContext.Exchanges.AsNoTracking()
            .FirstOrDefaultAsync(e => e.Name == "Binance", cancellationToken: cancellationToken);
